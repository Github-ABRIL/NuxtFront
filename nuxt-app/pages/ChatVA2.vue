        ...
        >
          ...
          <div class="message-name text-sm text-white">
            <span v-if="message.sender === 'Bot'" class="flex items-center">
              ...
              <span class="font-bold text-gray-300">{{ message.sender }}</span>
            </span>
            <span
  ...
  methods: {
    async loadMessages() {
      try {
        ...
        this.scrollMessagesToBottom(); // Desplazar hacia abajo después de cargar los mensajes
      } catch (error) {
     ...
     sendMessage() {
      ...
      if (message.trim() !== "") {
        // Agregar el nuevo mensaje a la conversación localmente
        ...
        try {
          // Enviar el nuevo mensaje al backend
           axios.post("/api/enviar-mensaje", {
            message: message,
            sender: "Persona",
          });
          // Desplazar la conversación hacia abajo para mostrar el nuevo mensaje
          this.scrollMessagesToBottom(); // Desplazar hacia abajo después de enviar un mensaje
        } catch (error) {
    ...
    scrollMessagesToBottom() {
      // Espera a que se actualice el DOM antes de desplazar
      this.$nextTick(() => {
