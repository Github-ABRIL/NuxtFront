<template>
  <div>
    <div class="text-effect-wrapper">
      <h1 class="text" contenteditable>{{ mensaje }}</h1>
    </div>
    <h3 contenteditable>{{ creditos }}</h3>
    <label for="option-toggle">
      <input type="checkbox" id="option-toggle" v-model="versionToggle"> 
    </label>
  </div>
</template>
...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      const checkbox = this.$el.querySelector('#option-toggle');
      checkbox.click(); // Simula hacer clic en el checkbox
    });
  }
};
</script>
...
h3 {
  ...
  border: none; /* Elimina cualquier borde alrededor del elemento */
